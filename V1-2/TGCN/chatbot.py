...
TEMPERATURE = 0.6          # keep answers focused
...
# ----- 3. Build request -------------------------------------------------------
async def chat_complete(words_list: str) -> str:
    """
    Asynchronously send `prompt` to the model with few-shot examples
    and return the assistant's reply.
    """

    #Secret from the Github Actions secrets
    api_key = os.getenv("API_KEY")

    if not api_key:
        raise EnvironmentError("OPENAI_API_KEY env-var not found")

    user_prompt = f"Your job is to reconstruct the user's sentence from a sequence of words. For each word, you will receive a list of the likeliest possibilities for the current word. You will essentially receive a list of most possible words for each word in the sentence. Please keep in mind that this comes in from a live camera feed, so try to look for the sentence that makes the most sense. Conclude from this: {words_list}. Make sure that the output is EXACTLY the predicted sentence and only that"
